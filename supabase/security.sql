...
-- ------------------------------------------------------------------
-- 2. Columns for rooms: creator + expiry
-- ------------------------------------------------------------------
alter table public.rooms
  add column if not exists creator_id uuid,
  add column if not exists expires_at timestamptz not null default now() + interval '12 hours';

-- Backfill creator_id from the host participant (test data only).
update public.rooms r
  set creator_id = (select p.id from public.participants p
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    ...
    where p.room_id = rid and p.id = auth.uid()
  );
$$;
...
create policy participants_select on public.participants for select
  using (id = auth.uid() or is_room_member(room_id));
...
create policy answers_insert on public.answers for insert
  with check (participant_id = auth.uid() and exists (
    select 1 from public.sessions s where s.id = answers.session_id and is_room_member(s.room_id)));

create policy answers_update on public.answers for update
  using (participant_id = auth.uid() and exists (
           select 1 from public.sessions s where s.id = answers.session_id and is_room_member(s.room_id)))
  with check (participant_id = auth.uid() and exists (
                select 1 from public.sessions s where s.id = answers.session_id and is_room_member(s.room_id)));
...
create policy discoveries_select on public.discoveries for select
  using (is_room_member(room_id) or saved_by = auth.uid());

create policy discoveries_insert on public.discoveries for insert
  with check (saved_by = auth.uid() and is_room_member(room_id));
